...
export default {
  ...
  onLoad() {
    ...
    const contextMenuUnpatch = patchBefore(
      "render",
      ...
      (args) => {
        try {
          ...
          if (props.options.some((option) => option?.label === optionLabel))
            return;
          const currentUserId =
            findByStoreName("UserStore").getCurrentUser()?.id;
          const focusedUserId =
            Object.keys(a._owner.stateNode._keyChildMapping)
              .find(
                (str) =>
              ...
              try {
                showToast(
                  focusedUserId === currentUserId
                    ? `Copied your token`
                    : `Copied token of ${props.header.title}`
                );
                setString(
                  focusedUserId === currentUserId
                    ? token
                    : [
                        Buffer.from(focusedUserId)
                          .toString("base64")
                          .replaceAll("=", ""), // thanks Marvin (@objectified <@562415519454461962>)
                ...
                alert(
                  `[TokenUtils → context menu patch → option onPress] failed. Patch ${
                    successful ? "dis" : "en"
                  }abled\n` + e.stack
                );
              }
          ...
          alert(
            `[TokenUtils → context menu patch] failed. Patch ${
              successful ? "dis" : "en"
            }abled\n` + e.stack
          );
        }
